...
class MatchingEngineIndexEndpoint(base.VertexAiResourceNounWithFutureManager):
    ...
    def match(
        ...
        Args:
            ...
            embedding_ids (str):
               Optional. If `queries` set, will use `queries` to do nearest
               neighbor search. If `queries` isn't set, will first uses
        ...
        if not queries and not embedding_ids:
            raise ValueError(
                "To find neighbors using matching engine,"
                "please specify `queries` or `embedding_ids`"
            )
        # Query by embedding_ids if queries not provided
        ...
        for query in queries:
            ...
            if query_by_id:
                request.embedding_id = query
            else:
                request.float_val.extend(query)
